...
abstract class NodeDAO[T <: NeoNode[A] : Mapper, A](implicit val Mapper: Mapper[T]) {
  ...
  def findById(a: A, label: Option[String] = None)(implicit connection: Neo4jREST, ec: ExecutionContext): Future[Try[T]] = {
    ...
    Future {
      val res = Cypher(query).as(get[org.anormcypher.NeoNode]("n") *).collectFirst({ case n => n }).map({ case n => Try(Mapper.mapToCase(n.props)) })
      res.getOrElse(Failure(new Exception("Error")))
    }
  }
